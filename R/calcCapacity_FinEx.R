...
calcCapacity <- function(subtype){
  if (subtype == "capacityByTech_windoff") {
    ...
    output <- new.magpie(cells_and_regions=c(getRegions(out)), years = c(2010,2015,2020), names = c("pecoal","pegas","pebiolc","pehyd"), fill=0)
  ...
  }else if (subtype=="coal2030") {
    output <- readSource("GCPT",subtype="REM_PEGASOS_pubfinex") * 1e-03
    description <- "Coal power capacity scenarios for 2030 following overseas public finance exit"
    
  }else if (subtype=="coal_finEx") {
    output <- readSource("GCPT",subtype="G20_FinEx_2020") * 1e-03
    description <- "Coal power capacity scenarios for 2025 following G20 overseas public finance exit"
