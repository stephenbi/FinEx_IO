finEx_reg_mag_nm_gcpt20 <- mselect(reg_mag_nm_gcpt20,Country=getItems(reg_mag_nm_gcpt20,1)[which(reg_mag_nm_gcpt20[,,"PreCon.PubG20.GW"]>0)])
finEx_glo_mag_nm_gcpt20 <- setItems(dimSums(finEx_reg_mag_nm_gcpt20,dim=1),dim=1,value="GLO")
  
q_finEx_by_host_reg <- as.quitte(mbind(finEx_reg_mag_nm_gcpt20,finEx_glo_mag_nm_gcpt20)) 
...
q_finEx_cap_by_host_reg_precon %>% 
  ...
  scale_alpha_manual(values=stackOrdAlpha, labels = c("Halted", "Active"), name = "Finance Status") +
...
for (reg in sort(host_regi)) {
  ...
  theme(title = element_blank(),
    ...
    axis.text.x = element_text(size = 2 * 9),
        ...
        strip.text = element_text(size = 2 * 10), 
    strip.background = element_rect(fill = rev(brewer.pal(n = 12, name = "Set3"))[which(sort(unique(map$RegionCode)) == reg)]),
        legend.text = element_text(size = 2 * 9),
        legend.title = element_text(size = 2 * 9, face="bold"),
